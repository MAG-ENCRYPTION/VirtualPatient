...
import {createFeedback, getClinicalCase, getExperts, verifySymptoms} from '../network'
...
export const EvaluationDetailReport = () => {
  ...
  useEffect(() => {
    console.log('VOICI LES EVALUATIONS ICI')
    ...
    getExperts()
      .then(experts => {
        ...
        for (const expert of experts) {
          newDictionary[expert.id] = expert.first_name;
        }
      ...
      {evaluation.feedbacks ? (
        <div>
          {evaluation.feedbacks.map((feed) => (
            <Feedback
              key={feed.id}
              author={getExpertByIdInDictionary(extractIdFromUrl(feed?.expert_physician?.toString()) || "")}
